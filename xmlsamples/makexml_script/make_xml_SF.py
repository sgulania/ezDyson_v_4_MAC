...
while line:
    ...
    if (line.find('Standard Nuclear Orientation')>=0):
        ...
        while line.find('----') == -1:
            ...
            geometry+=line[5:]
            line=qchemF.readline()
    if (line.find('EOM-IP-CCSD state')!=-1 or line.find('EOM-EA-CCSD state')!=-1):
...
print 'ALL Dyson orbitals are included, starting from high spin reference to IP/EA'
print 'Order: Ref-IP1, Ref-IP2, ..., SF1-IP1, SF1-IP2, ..., SF2-IP1, SF2-IP2, ...; EA1-Ref, EA2-Ref, ..., EA1-SF1, EA2-SF1, ..., EA1-SF2, EA2-SF2, ...'
print 'Restriction: if you happened to request both EOM-SF and EOM-EE transitions in the same Q-Chem job, this script will process all EE transitioins, too. Please pay attention to orbital labeles in the resulting xml then.'
print 'Please update spin_degeneracy and orbital_degeneracy to account for degeneracy factors (see ezDyson manual for details). For closed shell systems, spin_degeneracy must be set to 2.'
