...
while line:
    if (line.find("Molden")!=-1):
        n_dyson_jobs=1
    line=qchemF.readline()
qchemF.close()
print 'Dyson jobs in Q-Chem output: '+str(n_dyson_jobs)
if (n_dyson_jobs!=1):
    print 'Error: Q-Chem output should contain molden\n'
    sys.exit()
...
#        Next 2 lines added to avoid ezDyson crashing for molecules with Cs symmetry with A' and A" irreps
     xmlF.write("""  <DMO   norm=\""""+str(d_norm)+"""\"    transition=\""""+str(dyson_transition+1)+"""\"    comment=\"dyson right\"\n    text=\"\n""")
     line=qchemF.readline()
     while (line.find('Occup=1')==-1) and line:
         line=qchemF.readline()
     line=qchemF.readline()
     while (line.find('Sym=X')==-1):
         xmlF.write(line[6:])
         line=qchemF.readline()
     xmlF.write("""    \"  />\n\n""")
     xmlF.write("""  <DMO   norm=\""""+str(d_norm)+"""\"    transition=\""""+str(dyson_transition+1)+"""\"    comment=\"dyson left\"\n    text=\"\n""")
     line=qchemG.readline()
     while (line.find('Occup=1')==-1) and line:
         line=qchemG.readline()
     line=qchemG.readline()
     while (line.find('Sym=X')==-1):
...
if (number_MOs!=0):
    ...
    qchemF=open(sys.argv[2],'r')
    for MO_number in range(number_MOs):
        #copy alpha MOs
        xmlF.write("""    <MO                          type="alpha" number=\""""+str(MO_number+1)+"""\"   text=\"\n""")
